...
export const loader = async ({ request }: LoaderFunctionArgs) => {
    ...
    const response = await admin.graphql(
        `#graphql
    query List { metaobjects(type: "$app:sign3d_finish", first: 100) { edges { node { id fields { key value } } } } }`
    );
    ...
    if (intent === "create") {
      ...
      mutation Create($metaobject: MetaobjectCreateInput!) {
        metaobjectCreate(metaobject: $metaobject) { metaobject { id } userErrors { field message } }
            ...
            { variables: { metaobject: { type: "$app:sign3d_finish", fields } } }
        );
        const data = await response.json();
            ...
            <s-section heading="Add New Finish Type">
                ...
                <fetcher.Form method="post">
                    ...
                    <s-stack direction="inline" gap="base">
                        <input type="text" name="label" placeholder="Label (e.g. Matte)" required />
