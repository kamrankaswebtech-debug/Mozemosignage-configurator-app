...
export const loader = async ({ request }: LoaderFunctionArgs) => {
    ...
    const response = await admin.graphql(
        `#graphql
    query List { metaobjects(type: "$app:sign3d_front_colour", first: 100) { edges { node { id fields { key value } } } } }`
    );
    ...
    if (intent === "create") {
      ...
      mutation Create($metaobject: MetaobjectCreateInput!) {
        metaobjectCreate(metaobject: $metaobject) { metaobject { id } userErrors { field message } }
            ...
            { variables: { metaobject: { type: "$app:sign3d_front_colour", fields } } }
        );
        const data = await response.json();
...
export default function FrontColoursPage() {
    ...
    return (
        <s-page heading="3D Sign Front Colours">
            <s-section heading="Add New Front Colour">
