...
export const loader = async ({ request }: LoaderFunctionArgs) => {
    ...
    const items: Entry[] = all.filter((a: any) => a.category === "letter_count_tier");
...
export const action = async ({ request }: ActionFunctionArgs) => {
    ...
    const fields = [
        ...
        { key: "category", value: "letter_count_tier" },
    ];
...
export default function LetterCountTiersPage() {
    ...
    return (
        <s-page heading="Letter / Character Count Pricing Tiers">
            <s-section heading="Add New Letter Count Tier">
                <s-link href="/app/3d-signs">← Back to 3D Illuminated Signs</s-link>
                <s-paragraph>
                    Only applies in "Type Wording" mode (character count comes from the typed text). "Starts At"
                    = the minimum letter count this tier's surcharge begins at. Storefront finds the HIGHEST tier
                    whose "Starts At" is less than or equal to the customer's letter count, and applies its Surcharge.
                    Example rows: Starts At 1 → $0 (included), Starts At 21 → $100, Starts At 31 → $200, etc.
                </s-paragraph>
                <fetcher.Form method="post">
                    <input type="hidden" name="intent" value="create" />
                    <s-stack direction="inline" gap="base">
                        <input type="text" name="label" placeholder="Label (e.g. 21-30 letters)" required />
                        <input type="number" name="thresholdValue" placeholder="Starts At (letter count)" required />
                        <input type="number" step="0.01" name="extraPrice" placeholder="Surcharge ($)" defaultValue="0" />
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                        <tr style={{ textAlign: "left", borderBottom: "1px solid #ccc" }}>
                            ...
                            <th style={{ padding: "8px" }}>Starts At</th>
                            <th style={{ padding: "8px" }}>Surcharge</th>
                            ...
                            <tr key={item.id} style={{ borderBottom: "1px solid #eee" }}>
                                {editingId === item.id ? (
                                    <td colSpan={5} style={{ padding: "8px" }}>
                                        <fetcher.Form method="post">
                                            ...
                                            <s-stack direction="inline" gap="base">
                                                ...
                                                <input type="number" name="thresholdValue" defaultValue={item.thresholdValue} required />
                                    ...
                                    <>
                                        ...
                                        <td style={{ padding: "8px" }}>{item.thresholdValue}</td>
