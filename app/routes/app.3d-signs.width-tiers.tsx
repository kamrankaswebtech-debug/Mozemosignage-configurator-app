...
export const loader = async ({ request }: LoaderFunctionArgs) => {
    ...
    const items: Entry[] = all.filter((a: any) => a.category === "width_tier");
...
export default function WidthTiersPage() {
    ...
    return (
        <s-page heading="Width / Length Starting Price Tiers">
            <s-section heading="Add New Width Tier">
                <s-link href="/app/3d-signs">← Back to 3D Illuminated Signs</s-link>
                <s-paragraph>
                    Each entry = one width breakpoint (e.g. 50cm, 75cm, 1m...). The storefront picks the tier whose
                    Width (cm) is the smallest value that is greater than or equal to the customer's entered width.
                    "Starting Price" is the FULL starting price at that width (standard 25cm letters, indoor, front-lit).
                </s-paragraph>
                <fetcher.Form method="post">
                    <input type="hidden" name="intent" value="create" />
                    <s-stack direction="inline" gap="base">
                        <input type="text" name="label" placeholder="Label (e.g. 50 cm)" required />
                        <input type="number" step="0.01" name="thresholdValue" placeholder="Width (cm)" required />
                        <input type="number" step="0.01" name="extraPrice" placeholder="Starting Price ($)" defaultValue="0" />
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                        <tr style={{ textAlign: "left", borderBottom: "1px solid #ccc" }}>
                            ...
                            <th style={{ padding: "8px" }}>Starting Price</th>
