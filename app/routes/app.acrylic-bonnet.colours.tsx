...
export const loader = async ({ request }: LoaderFunctionArgs) => {
    ...
    query ListBonnetColours { metaobjects(type: "$app:bonnet_option", first: 100) { edges { node { id fields { key value } } } } }`);
    ...
    const colours: ColourEntry[] = all.filter((a: any) => a.category === "colour");
...
export const action = async ({ request }: ActionFunctionArgs) => {
    ...
    const fields = [
        ...
        { key: "category", value: "colour" },
    ];

    if (intent === "create") {
        const response = await admin.graphql(`#graphql
      mutation CreateBonnetColour($metaobject: MetaobjectCreateInput!) { metaobjectCreate(metaobject: $metaobject) { metaobject { id } userErrors { field message } } }`,
    ...
    if (intent === "update") {
        ...
        const response = await admin.graphql(`#graphql
      mutation UpdateBonnetColour($id: ID!, $metaobject: MetaobjectUpdateInput!) { metaobjectUpdate(id: $id, metaobject: $metaobject) { metaobject { id } userErrors { field message } } }`,
    ...
    if (intent === "delete") {
        const id = String(formData.get("id"));
        const response = await admin.graphql(`#graphql
      mutation DeleteBonnetColour($id: ID!) { metaobjectDelete(id: $id) { deletedId userErrors { field message } } }`,
            { variables: { id } });
        const data = await response.json();
...
export default function BonnetColoursPage() {
    ...
    return (
        <s-page heading="Acrylic Neon Bonnet — Face Colours">
            <s-section heading="Add New Face Colour">
                <s-link href="/app/acrylic-bonnet">← Back to Acrylic Neon Bonnet</s-link>
                <s-paragraph>Per the client's PDF: Black, White, Light Grey, Dark Grey, Silver, Red, Orange, Yellow, Light Blue, Dark Blue/Navy, Green, Dark Green, Purple, Pink, Maroon, Custom Colour.</s-paragraph>
                <fetcher.Form method="post">
                    <input type="hidden" name="intent" value="create" />
                    <s-stack direction="inline" gap="base">
                        <input type="text" name="label" placeholder="Label (e.g. Black)" required />
                        <input type="text" name="hexCode" placeholder="#000000 (leave blank for Custom Colour)" />
            ...
            <s-section heading={`Existing Face Colours (${colours.length})`}>
