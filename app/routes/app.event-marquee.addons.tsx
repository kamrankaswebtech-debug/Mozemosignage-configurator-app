...
type Entry = { id: string; label: string; extraPrice: string; sortOrder: string; isFree: boolean };
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
    ...
    const items: Entry[] = data.data.metaobjects.edges
        .map((edge: any) => {
            ...
            return { id: edge.node.id, category: f.category, label: f.label || "", extraPrice: f.extra_price_decimal || "0", sortOrder: f.sort_order || "0", isFree: f.is_free === "true" };
        })
...
export const action = async ({ request }: ActionFunctionArgs) => {
    ...
    const fields = [
        ...
        { key: "is_free", value: String(formData.get("isFree") === "on") },
            ...
            <s-section heading="Add New Add-on">
                ...
                <fetcher.Form method="post">
                    ...
                    <s-stack direction="inline" gap="base">
                        <input type="text" name="label" placeholder="Label (e.g. Remote Dimmer)" required />
                        ...
                        {items.map((item) => (
                            <tr key={item.id} style={{ borderBottom: "1px solid #eee" }}>
                                {editingId === item.id ? (
                                    ...
                                    <>
                                        <td style={{ padding: "8px" }}>{item.label}{item.isFree ? " (Free)" : ""}</td>
