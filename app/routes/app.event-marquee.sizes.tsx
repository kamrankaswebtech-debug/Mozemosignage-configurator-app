...
const CATEGORY = "size";
type Entry = { id: string; label: string; heightCm: string; price: string; sortOrder: string };
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
    ...
    const items: Entry[] = data.data.metaobjects.edges
        .map((edge: any) => {
            ...
            return { id: edge.node.id, category: f.category, label: f.label || "", heightCm: f.height_cm || "0", price: f.extra_price_decimal || "0", sortOrder: f.sort_order || "0" };
        })
...
export const action = async ({ request }: ActionFunctionArgs) => {
    ...
    const fields = [
        ...
        { key: "extra_price_decimal", value: String(formData.get("price") || "0") },
...
export default function SizesPage() {
    ...
    return (
        <s-page heading="Sizes">
            <s-section heading="Add New Size">
                <s-link href="/app/event-marquee">← Back to Event Marquee</s-link>
                <fetcher.Form method="post">
                    <input type="hidden" name="intent" value="create" />
                    <s-stack direction="inline" gap="base">
                        <input type="text" name="label" placeholder="Label (e.g. Standard - 60cm tall)" required />
                        <input type="number" step="0.01" name="heightCm" placeholder="Height (cm)" required />
                        <input type="number" step="0.01" name="price" placeholder="Price" defaultValue="0" />
                            ...
                            <tr key={item.id} style={{ borderBottom: "1px solid #eee" }}>
                                {editingId === item.id ? (
                                    <td colSpan={5} style={{ padding: "8px" }}>
                                        <fetcher.Form method="post">
                                            ...
                                            <s-stack direction="inline" gap="base">
                                                ...
                                                <input type="number" step="0.01" name="heightCm" defaultValue={item.heightCm} required />
                                    ...
                                    <>
                                        ...
                                        <td style={{ padding: "8px" }}>{item.heightCm} cm</td>
