...
export const loader = async ({ request }: LoaderFunctionArgs) => {
    ...
    const items: Entry[] = data.data.metaobjects.edges
        .map((edge: any) => {
            ...
            return { id: edge.node.id, category: f.category, label: f.label || "", extraPrice: f.extra_price_decimal || "0", sortOrder: f.sort_order || "0" };
        })
...
export default function StylesPage() {
    ...
    return (
        <s-page heading="Letter Styles">
            <s-section heading="Add New Letter Style">
                <s-link href="/app/event-marquee">← Back to Event Marquee</s-link>
                <fetcher.Form method="post">
                    <input type="hidden" name="intent" value="create" />
                    <s-stack direction="inline" gap="base">
                        <input type="text" name="label" placeholder="Label (e.g. Vintage Metal)" required />
