...
export const loader = async ({ request }: LoaderFunctionArgs) => {
    ...
    const response = await admin.graphql(
        `#graphql
    query List { metaobjects(type: "$app:infinity_colour", first: 100) { edges { node { id fields { key value } } } } }`
    );
    const data = await response.json();
    const items: Entry[] = data.data.metaobjects.edges.map((edge: any) => {
        const f: Record<string, string> = {};
        edge.node.fields.forEach((x: any) => { f[x.key] = x.value; });
        return { id: edge.node.id, name: f.name || "", hexCode: f.hex_code || "#ffffff", extraPrice: f.extra_price_decimal || "0", sortOrder: f.sort_order || "0" };
    });
    ...
    if (intent === "create") {
      ...
      mutation Create($metaobject: MetaobjectCreateInput!) {
        metaobjectCreate(metaobject: $metaobject) { metaobject { id } userErrors { field message } }
            ...
            { variables: { metaobject: { type: "$app:infinity_colour", fields } } }
        );
        const data = await response.json();
...
export default function ColoursPage() {
    ...
    return (
        <s-page heading="LED Colours">
            <s-section heading="Add New LED Colour">
                <s-link href="/app/infinity-mirror">← Back to Infinity Mirror</s-link>
                <fetcher.Form method="post">
                    <input type="hidden" name="intent" value="create" />
                    <s-stack direction="inline" gap="base">
                        <input type="text" name="name" placeholder="Name (e.g. Warm White)" required />
