...
type Entry = { id: string; label: string; widthCm: string; price: string; sortOrder: string };

export const loader = async ({ request }: LoaderFunctionArgs) => {
    const { admin } = await authenticate.admin(request);
    const response = await admin.graphql(
        `#graphql
    query List { metaobjects(type: "$app:infinity_size", first: 100) { edges { node { id fields { key value } } } } }`
    );
    const data = await response.json();
    const items: Entry[] = data.data.metaobjects.edges.map((edge: any) => {
        const f: Record<string, string> = {};
        edge.node.fields.forEach((x: any) => { f[x.key] = x.value; });
        return { id: edge.node.id, label: f.label || "", widthCm: f.width_cm || "0", price: f.price_decimal || "0", sortOrder: f.sort_order || "0" };
    });
    ...
    if (intent === "create") {
      ...
      mutation Create($metaobject: MetaobjectCreateInput!) {
        metaobjectCreate(metaobject: $metaobject) { metaobject { id } userErrors { field message } }
            ...
            { variables: { metaobject: { type: "$app:infinity_size", fields } } }
        );
        const data = await response.json();
    ...
    return (
        <s-page heading="Sizes">
            <s-section heading="Add New Size">
                <s-link href="/app/infinity-mirror">← Back to Infinity Mirror Letter Signs</s-link>
                <fetcher.Form method="post">
                    <input type="hidden" name="intent" value="create" />
                    <s-stack direction="inline" gap="base">
                        <input type="text" name="label" placeholder="Label (e.g. Medium - 60cm wide)" required />
                        <input type="number" step="0.01" name="widthCm" placeholder="Width (cm)" required />
                            ...
                            <tr key={item.id} style={{ borderBottom: "1px solid #eee" }}>
                                {editingId === item.id ? (
                                    <td colSpan={5} style={{ padding: "8px" }}>
                                        <fetcher.Form method="post">
                                            ...
                                            <s-stack direction="inline" gap="base">
                                                ...
                                                <input type="number" step="0.01" name="widthCm" defaultValue={item.widthCm} required />
                                    ...
                                    <>
                                        ...
                                        <td style={{ padding: "8px" }}>{item.widthCm} cm</td>
