...
export const loader = async ({ request }: LoaderFunctionArgs) => {
    ...
    const response = await admin.graphql(
        `#graphql
    query ListBackboardColours {
      metaobjects(type: "$app:backboard_colour", first: 100) {
        edges { node { id fields { key value } } }
      }
    }`
    );
    const data = await response.json();
    const colours: BackboardColourEntry[] = data.data.metaobjects.edges.map((edge: any) => {
        const f: Record<string, string> = {};
        edge.node.fields.forEach((x: any) => { f[x.key] = x.value; });
        return {
            id: edge.node.id,
            label: f.label || "",
            swatch: f.hex_or_swatch || "",
...
export const action = async ({ request }: ActionFunctionArgs) => {
    ...
    const fields = [
        ...
        { key: "hex_or_swatch", value: String(formData.get("swatch") || "") },
...
export default function BackboardColoursPage() {
    ...
    return (
        <s-page heading="Backboard Colours">
            <s-section heading="Add New Backboard Colour">
                ...
                <fetcher.Form method="post">
                    ...
                    <s-stack direction="inline" gap="base">
                        <input type="text" name="label" placeholder="Label (e.g. Gloss Black)" required />
                        <input type="text" name="swatch" placeholder="#000000" required />
            ...
            <s-section heading={`Existing Backboard Colours (${colours.length})`}>
                                ...
                                {editingId === c.id ? (
                                    <td colSpan={5} style={{ padding: "8px" }}>
                                        <fetcher.Form method="post">
                                            ...
                                            <s-stack direction="inline" gap="base">
                                                ...
                                                <input type="text" name="swatch" defaultValue={c.swatch} required />
                                    ...
                                    <>
                                        <td style={{ padding: "8px" }}>
                                            <span style={{ display: "inline-block", width: 14, height: 14, border: "1px solid #ccc", background: c.swatch, marginRight: 8, verticalAlign: "middle" }}></span>
                                            {c.label}
                                        </td>
                                            ...
                                            <s-stack direction="inline" gap="tight">
                                                ...
                                                <fetcher.Form method="post">
                                                    ...
                                                    <s-button variant="tertiary" tone="critical" type="submit"
                                                        onClick={(e) => { if (!confirm(`Delete "${c.label}"?`)) e.preventDefault(); }}>
                                                        Delete
                                                    </s-button>
                                                </fetcher.Form>
                                            </s-stack>
