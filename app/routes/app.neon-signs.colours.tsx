...
export const loader = async ({ request }: LoaderFunctionArgs) => {
    ...
    const response = await admin.graphql(
        `#graphql
    query ListNeonColours {
      metaobjects(type: "$app:neon_colour", first: 100) {
        edges { node { id fields { key value } } }
      }
    }`
    );
    const data = await response.json();
    const colours: ColourEntry[] = data.data.metaobjects.edges.map((edge: any) => {
...
export const action = async ({ request }: ActionFunctionArgs) => {
    ...
    if (intent === "create") {
        const response = await admin.graphql(
            `#graphql
      mutation CreateNeonColour($metaobject: MetaobjectCreateInput!) {
        metaobjectCreate(metaobject: $metaobject) {
          metaobject { id }
          userErrors { field message }
        }
      }`,
            { variables: { metaobject: { type: "$app:neon_colour", fields } } }
        );
        const data = await response.json();
    ...
    if (intent === "update") {
        ...
        const response = await admin.graphql(
            `#graphql
      mutation UpdateNeonColour($id: ID!, $metaobject: MetaobjectUpdateInput!) {
    ...
    if (intent === "delete") {
        ...
        const response = await admin.graphql(
            `#graphql
      mutation DeleteNeonColour($id: ID!) {
...
export default function NeonColoursPage() {
    ...
    return (
        <s-page heading="Neon Colours">
            <s-section heading="Add New Colour">
                <s-link href="/app/neon-signs">← Back to Neon Signs</s-link>
                <fetcher.Form method="post">
                    <input type="hidden" name="intent" value="create" />
                    <s-stack direction="inline" gap="base">
                        <input type="text" name="name" placeholder="Name (e.g. Hot Pink)" required />
                        <input type="text" name="hexCode" placeholder="#FF1493" required />
            ...
            <s-section heading={`Existing Colours (${colours.length})`}>
                                ...
                                {editingId === c.id ? (
                                    <td colSpan={5} style={{ padding: "8px" }}>
                                        <fetcher.Form method="post">
                                            ...
                                            <s-stack direction="inline" gap="base">
                                                ...
                                                <input type="text" name="hexCode" defaultValue={c.hexCode} required />
                                    ...
                                    <>
                                        <td style={{ padding: "8px" }}>
                                            <span style={{ display: "inline-block", width: 14, height: 14, borderRadius: "50%", background: c.hexCode, marginRight: 8, verticalAlign: "middle" }}></span>
                                            {c.name}
                                        </td>
                                        <td style={{ padding: "8px" }}>{c.hexCode}</td>
                                            ...
                                            <s-stack direction="inline" gap="tight">
                                                ...
                                                <fetcher.Form method="post">
                                                    ...
                                                    <s-button variant="tertiary" tone="critical" type="submit"
                                                        onClick={(e) => { if (!confirm(`Delete colour "${c.name}"?`)) e.preventDefault(); }}>
                                                        Delete
                                                    </s-button>
                                                </fetcher.Form>
                                            </s-stack>
