...
export const loader = async ({ request }: LoaderFunctionArgs) => {
    ...
    const response = await admin.graphql(
        `#graphql
    query ListPowerAdapters {
      metaobjects(type: "$app:power_adapter", first: 100) {
        edges { node { id fields { key value } } }
      }
    }`
    );
    const data = await response.json();
    const adapters: AdapterEntry[] = data.data.metaobjects.edges.map((edge: any) => {
    ...
    adapters.sort((a, b) => Number(a.sortOrder) - Number(b.sortOrder));
    return { adapters };
};
...
export const action = async ({ request }: ActionFunctionArgs) => {
    ...
    if (intent === "create") {
        const response = await admin.graphql(
            `#graphql
      mutation CreatePowerAdapter($metaobject: MetaobjectCreateInput!) {
        metaobjectCreate(metaobject: $metaobject) {
          metaobject { id }
          userErrors { field message }
        }
      }`,
            { variables: { metaobject: { type: "$app:power_adapter", fields } } }
        );
        const data = await response.json();
    ...
    if (intent === "update") {
        ...
        const response = await admin.graphql(
            `#graphql
      mutation UpdatePowerAdapter($id: ID!, $metaobject: MetaobjectUpdateInput!) {
    ...
    if (intent === "delete") {
        ...
        const response = await admin.graphql(
            `#graphql
      mutation DeletePowerAdapter($id: ID!) {
...
export default function PowerAdaptersPage() {
    const { adapters } = useLoaderData<typeof loader>();
    ...
    return (
        <s-page heading="Power Adapters">
            <s-section heading="Add New Power Adapter">
                <s-link href="/app/neon-signs">← Back to Neon Signs</s-link>
                <fetcher.Form method="post">
                    <input type="hidden" name="intent" value="create" />
                    <s-stack direction="inline" gap="base">
                        <input type="text" name="label" placeholder="e.g. Australia / New Zealand 230V" required />
                        <input type="number" name="sortOrder" placeholder="Sort Order" defaultValue={adapters.length + 1} />
                        <s-button type="submit" {...(isSubmitting ? { loading: true } : {})}>Add</s-button>
                    </s-stack>
                </fetcher.Form>
            </s-section>

            <s-section heading={`Existing Power Adapters (${adapters.length})`}>
