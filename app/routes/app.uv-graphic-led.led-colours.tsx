...
type Entry = { id: string; name: string; hexCode: string; extraPrice: string; sortOrder: string };

export const loader = async ({ request }: LoaderFunctionArgs) => {
    const { admin } = await authenticate.admin(request);
    const response = await admin.graphql(
        `#graphql
    query List { metaobjects(type: "$app:uv_led_colour", first: 100) { edges { node { id fields { key value } } } } }`
    );
    const data = await response.json();
    const items: Entry[] = data.data.metaobjects.edges.map((edge: any) => {
        const f: Record<string, string> = {};
        edge.node.fields.forEach((x: any) => { f[x.key] = x.value; });
        return { id: edge.node.id, name: f.name || "", hexCode: f.hex_code || "", extraPrice: f.extra_price_decimal || "0", sortOrder: f.sort_order || "0" };
    });
    ...
    if (intent === "create") {
      ...
      mutation Create($metaobject: MetaobjectCreateInput!) {
        metaobjectCreate(metaobject: $metaobject) { metaobject { id } userErrors { field message } }
            ...
            { variables: { metaobject: { type: "$app:uv_led_colour", fields } } }
        );
        const data = await response.json();
...
export default function UvLedColoursPage() {
    ...
    return (
        <s-page heading="UV LED Colours">
            <s-section heading="Add New UV LED Colours">
                <s-link href="/app/uv-graphic-led">← Back to UV Graphic LED Signs</s-link>
                <fetcher.Form method="post">
                    <input type="hidden" name="intent" value="create" />
                    <s-stack direction="inline" gap="base">
                        <input type="text" name="name" placeholder="Name (e.g. Frosted White)" required />
                        <input type="text" name="hexCode" placeholder="#FFFFFF" required />
                                ...
                                {editingId === item.id ? (
                                    ...
                                    <>
                                        <td style={{ padding: "8px" }}>
                                            <span style={{ display: "inline-block", width: 14, height: 14, border: "1px solid #ccc", borderRadius: "50%", background: item.hexCode, marginRight: 8, verticalAlign: "middle" }}></span>
                                            {item.name}
                                        </td>
                                        <td style={{ padding: "8px" }}>{item.hexCode}</td>
