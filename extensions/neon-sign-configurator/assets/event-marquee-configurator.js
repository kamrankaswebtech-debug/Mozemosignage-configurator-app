// Mozemo Signage - Event Marquee Letter Signs Configurator: live preview + dynamic pricing
function initAllMarqueeConfigurators() {
    document.querySelectorAll('.marquee-configurator').forEach((root) => {
        if (root.dataset.marqueeConfiguratorInitialized === 'true') return;
        try {
            initMarqueeConfigurator(root);
            root.dataset.marqueeConfiguratorInitialized = 'true';
        } catch (err) {
            console.error('Event Marquee Configurator: init failed for this block:', err, root);
        }
    });
}

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initAllMarqueeConfigurators);
} else {
    initAllMarqueeConfigurators();
}

document.addEventListener('shopify:section:load', initAllMarqueeConfigurators);
document.addEventListener('cart:refresh', initAllMarqueeConfigurators);
...
function setButtonLoadingText(button, text) {
    button.innerHTML = '<span class="marquee-configurator__spinner"></span><span>' + text + '</span>';
}
...
function initMarqueeConfigurator(root) {
    ...
    const swatches = root.querySelectorAll('[data-colour-swatches] .marquee-configurator__swatch');
    ...
    function calculateTotal() {
        let total = 0;
        const selects = [styleSelect, sizeSelect, mountingSelect];
    ...
    [styleSelect, sizeSelect, mountingSelect].forEach((select) => {
    ...
    if (addToCartBtn) {
        addToCartBtn.addEventListener('click', async () => {
            ...
            if (!variantId) {
                console.error('Event Marquee Configurator: No variant ID found on Add to Cart button.');
            ...
            const properties = {
                ...
                'Letter Style': styleSelect.options[styleSelect.selectedIndex].textContent.trim(),
                'Size': sizeSelect.options[sizeSelect.selectedIndex].textContent.trim(),
                'Bulb Colour': colourNameLabel.textContent.trim(),
            ...
            try {
                ...
                try {
                    ...
                    document.dispatchEvent(new themeEvents.CartAddEvent({}, 'event-marquee-configurator', {
                    ...
                    console.warn('Event Marquee Configurator: could not notify theme cart UI.', themeEventError);
                }

                document.dispatchEvent(new CustomEvent('cart:refresh'));
            } catch (error) {
                console.error('Event Marquee Configurator Add to Cart error:', error);
